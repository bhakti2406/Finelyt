...
if st.session_state.logged_in and 'page' in params:
    ...
    _nav_items_check = [
        "Dashboard", "Add Expense", "Income", "Budgets", "Anomalies", "AI Coach",
        "Financial Health", "Financial Forecast", "Risk Engine", "Wealth Module",
        "Net Worth", "Compliance & Security", "Savings",
        "Streaks", "Bill Calendar", "Reports", "Summary", "Settings"
    ]
        ...
        with st.container(key="login_panel"):
            if st.session_state.auth_step == "LOCKED":
                ...
                <div style='text-align:center; margin-bottom: 1.5rem;'>
                    ...
                    <div style='margin: 1rem auto; max-width: 320px; padding: 10px 14px; border: 1px solid rgba(255, 179, 71, 0.35); background: rgba(255, 179, 71, 0.05); border-radius: 8px; font-size: 0.88rem; color: #8B8FA8; letter-spacing: 0.05em;'>
                ...
                with tab_login:
                    ...
                    with st.form("login_form"):
                        ...
                        remember_me = st.checkbox("Remember me on this device", key="remember_me_check")
                        st.markdown("<br>", unsafe_allow_html=True)
                    ...
                    with col2:
                        if st.button("Reset Password", key="forgot_pwd_btn", use_container_width=False):
                    ...
                    if login_btn:
                        if not username or not password:
                            ...
                            allowed, rate_msg = check_rate_limit("login", 5, 900)
                            if not allowed:
                                st.error(rate_msg)
                            else:
                ...
                with tab_register:
                    ...
                    if register_btn:
                        ...
                        if not nu or not ne or not np or not nc:
                            ...
                            from utils.auth import check_rate_limit
                            allowed, rate_msg = check_rate_limit("register", 5, 900)
                            if not allowed:
                                st.error(rate_msg)
                            else:
                                ...
                                if not success:
                                    ...
                                    if not ok:
                                        # Email delivery failed — show generic message
                                        st.error(
...
_nav_items = [
    ...
    "Financial Health", "Financial Forecast", "Risk Engine", "Wealth Module",
    "Net Worth", "Compliance & Security", "Savings",
    "Streaks", "Bill Calendar", "Reports", "Summary"
]
...
def render_mega_nav():
    ...
    def get_svg_icon(name):
        svg = ICONS.get(name, '')
        return f'<span class="dropdown-list-icon">{svg}</span>'

    finance_items = [
        {"name": "Add Expense", "icon": "add", "desc": "Record and categorize daily expenses."},
        {"name": "Income", "icon": "income", "desc": "Track your incoming revenues."},
        {"name": "Budgets", "icon": "budget", "desc": "Set limits and check category spending."},
        {"name": "Savings", "icon": "savings", "desc": "Manage your savings goals."},
        {"name": "Bill Calendar", "icon": "calendar", "desc": "View and track upcoming bill dates."},
        {"name": "Net Worth", "icon": "bank", "desc": "Track your overall assets and liabilities."}
    ]

    ai_items = [
        {"name": "AI Coach", "icon": "ai", "desc": "Chat with your AI financial assistant."},
        {"name": "Financial Health", "icon": "streaks", "desc": "Analyze your overall financial score."},
        {"name": "Financial Forecast", "icon": "trending", "desc": "Predict future wealth & expenses."},
        {"name": "Risk Engine", "icon": "warning", "desc": "Real-time detection of financial risk factors."},
        {"name": "Wealth Module", "icon": "gem", "desc": "Advanced wealth growth and planning."},
        {"name": "Anomalies", "icon": "anomaly", "desc": "Discover and review budget leakages."}
    ]

    analytics_items = [
        {"name": "Reports", "icon": "reports", "desc": "Export customized financial statistics."},
        {"name": "Summary", "icon": "summary", "desc": "Quick executive overview of cashflows."},
        {"name": "Streaks", "icon": "flame", "desc": "Monitor days of financial discipline."}
    ]

    system_items = [
        {"name": "Compliance & Security", "icon": "shield", "desc": "Privacy standards and data protocols."}
    ]

    def make_cards(items):
        html = ""
        for item in items:
            active = active_class(item["name"])
            url_name = item["name"].replace(" ", "+").replace("&", "%26")
            html += f"""
            <a href="/?page={url_name}{token_param}" target="_self" class="dropdown-list-item {active}">
                {get_svg_icon(item["icon"])}
                <span class="dropdown-list-title">{item["name"]}</span>
            </a>
            """
        return "".join([l.strip() for l in html.splitlines() if l.strip()])

    finance_cards = make_cards(finance_items)
    ai_cards = make_cards(ai_items)
    analytics_cards = make_cards(analytics_items)
    system_cards = make_cards(system_items)

    home_active = "active" if current_page == "Dashboard" else ""

    html = f"""
    <div class="mega-nav">
        <label for="mobile-menu-cb" class="mobile-menu-toggle">☰</label>
        <input type="checkbox" id="mobile-menu-cb" />
        
        <div class="mega-nav-items-container">
            <div class="mega-nav-item">
                <a href="/?page=Dashboard{token_param}" target="_self" class="mega-nav-btn {home_active}">
                    {get_svg_icon('home')} Home
                </a>
            </div>

            <div class="mega-nav-item" tabindex="0">
                <button class="mega-nav-btn">
                    {get_svg_icon('cash')} Finance <span class="arrow">▼</span>
                </button>
            ...
            <div class="mega-nav-item" tabindex="0">
                <button class="mega-nav-btn">
                    {get_svg_icon('ai')} AI Intelligence <span class="arrow">▼</span>
                </button>
                <div class="mega-dropdown-panel">
                    {ai_cards}
                </div>
            ...
            <div class="mega-nav-item" tabindex="0">
                <button class="mega-nav-btn">
                    {get_svg_icon('bar_chart')} Analytics <span class="arrow">▼</span>
                </button>
                <div class="mega-dropdown-panel">
                    {analytics_cards}
                </div>
            ...
            <div class="mega-nav-item" tabindex="0">
                <button class="mega-nav-btn">
                    {get_svg_icon('settings')} System <span class="arrow">▼</span>
                </button>
                <div class="mega-dropdown-panel">
                    {system_cards}
                </div>
...
with st.container(key="global_header"):
    ...
    with col_center:
        st.markdown(render_mega_nav(), unsafe_allow_html=True)
...
if page == "Dashboard":
    ...
    import importlib
    from pages import health_score
    importlib.reload(health_score)
    health_score.show(user_id=user_id)
elif page == "Financial Forecast":
    import importlib
    from pages import forecasting
    importlib.reload(forecasting)
    forecasting.show(user_id=user_id)
elif page == "Risk Engine":
    from pages import risk_engine
    risk_engine.show(user_id=user_id)
elif page == "Wealth Module":
    from pages import wealth_module
    wealth_module.show(user_id=user_id)
elif page == "Compliance & Security":
    from pages import compliance
    compliance.show(user_id=user_id)
