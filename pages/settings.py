...
def show(user_id=1):
    ...
    with tab2:
        ...
        if save_pwd:
            if not current_pwd or not new_pwd or not confirm_pwd:
                ...
                success, msg = update_password(user_id, current_pwd, new_pwd)
                if success:
                    st.success(msg)
                else:
                    st.error(msg)
        ...
        <div style="background:var(--bg-card); border:1px solid var(--border);
            ...
            <p style="color:var(--text-secondary); font-size:0.88rem;
                ...
                encrypted credentials, and verified email security.
            </p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab4:
            ...
            with cd1:
                if st.button(
                    ...
                    if confirm_text == "DELETE":
                        ...
                        st.session_state.confirm_delete_data = False
                        st.success("All data deleted. Your account is now empty.")
                        st.rerun()
                    else:
        ...
        <div style="background:var(--danger-dim); border:1px solid var(--danger);
                    border-radius:12px; padding:1rem 1.2rem; margin-bottom:1rem;">
            <p style="color:var(--danger); font-weight:600; margin:0 0 4px 0;">
                🗑️ Delete My Account
            </p>
            <p style="color:var(--text-secondary); font-size:0.85rem; margin:0;">
                Permanently deletes your Finelyt account and
                <strong style="color:#FFFFFF;">all associated data</strong>
                including expenses, income, budgets, goals, bills, and settings.
                This action is <strong style="color:var(--danger);">
                irreversible</strong> and cannot be undone.
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        if not st.session_state.confirm_delete_account:
            ...
            # ── CONFIRMATION PANEL ────────────────────────────────────────────
            st.markdown("""
            <div style="background:#1A0A0A; border:1px solid var(--danger);
                        border-radius:12px; padding:1.2rem 1.5rem;
                        margin-bottom:1rem;">
                <p style="color:var(--danger); font-size:1rem;
                           font-weight:700; margin:0 0 8px 0;">
                    ⚠️ Delete Your Finelyt Account?
                </p>
                <p style="color:var(--text-secondary); font-size:0.85rem;
                           margin:0 0 12px 0; line-height:1.6;">
                    This will permanently remove:
                </p>
                <ul style="color:var(--text-secondary); font-size:0.83rem;
                            margin:0 0 12px 0; padding-left:1.2rem;
                            line-height:1.8;">
                    <li>Profile &amp; account information</li>
                    <li>All income &amp; expense records</li>
                    <li>Budgets &amp; savings goals</li>
                    <li>Recurring bills &amp; payments</li>
                    <li>Notifications &amp; activity logs</li>
                    <li>Settings &amp; preferences</li>
                    <li>All financial data associated with your account</li>
                </ul>
            ...
            st.markdown(
                "<p style='color:#8B8FA8; font-size:0.88rem;"
                "margin-bottom:4px;'>Enter your password to confirm:</p>",
                unsafe_allow_html=True
            )
            delete_pwd = st.text_input(
                "Password",
                type="password",
                placeholder="Enter your current password",
            ...
            st.markdown(
                "<p style='color:#8B8FA8; font-size:0.88rem;"
                "margin:12px 0 4px 0;'>Type <strong style='color:var(--danger);"
                "'>DELETE</strong> to confirm:</p>",
                unsafe_allow_html=True
            )
            ...
            with da2:
                if st.button(
                    ...
                    type="secondary",
                    use_container_width=True,
                    key="btn_confirm_delete_account"
                ):
                    # Validate inputs
                    if not delete_pwd:
                        st.error("Please enter your password.")
                    elif delete_confirm_text != "DELETE":
                        ...
                        if not row:
                        ...
                        elif not verify_password(delete_pwd, row[0]):
                            ...
                            from utils.db import delete_user_account
                            success, err = delete_user_account(
                                user_id, db_username, db_email
                            )
                            ...
                            if success:
                                ...
                                st.success(
                                    "✅ Your Finelyt account has been "
                                    "permanently deleted."
                                )
                                st.rerun()
                            else:
                                st.error("Account deletion failed. Please try again or contact support.")