...
def add_expense(date, category, amount, notes='', user_id=1):
    ...
    conn.execute(
        '''INSERT INTO expenses
           ...
           VALUES (?, ?, ?, ?, ?)''',
        (str(date), category, amount, notes, user_id)
    )
    conn.commit()
    conn.close()
    clear_cache()
...
def add_income(date, source, amount, notes='', user_id=1):
    ...
    conn.execute(
        '''INSERT INTO income
           ...
           VALUES (?, ?, ?, ?, ?)''',
        (str(date), source, amount, notes, user_id)
    )
    conn.commit()
    conn.close()
    clear_cache()
...
def delete_user_account(user_id: int, username: str, email: str) -> tuple[bool, str]:
    """
    ...
    try:
        ...
        # Explicit per-table DELETE statements (no f-string interpolation)
        for stmt in [
            ...
            "DELETE FROM saving_deposits WHERE user_id = ?",
            ...
            "DELETE FROM nw_assets WHERE user_id = ?",
            "DELETE FROM nw_liabilities WHERE user_id = ?",
            "DELETE FROM nw_history WHERE user_id = ?",
            "DELETE FROM sp_payments WHERE user_id = ?",
            "DELETE FROM settlements WHERE user_id = ?",
            "DELETE FROM health_score_history WHERE user_id = ?",
        ]:
            try:
        ...
        # login_attempts — keyed by username string AND email string
        try:
            conn.execute(
            ...
            if email:
                conn.execute(
                    "DELETE FROM login_attempts WHERE username = ?", (email,)
                )
        except Exception:
            pass
        ...
        logging.error("Account deletion failed for user_id=%s: %s", user_id, exc)
        conn.execute("ROLLBACK")
        conn.close()
        return False, "Account deletion failed. Please try again or contact support."