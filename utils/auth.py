...
if not _env_secret or len(_env_secret) < 32:
    logging.warning(
        "SECRET_KEY is missing or too short (< 32 chars) in .env. "
        "Generating a random ephemeral key — sessions will NOT survive restarts. "
...
def _smtp_configured() -> bool:
    """Returns True only when all required SMTP env vars are set."""
...
def verify_password(password, stored_hash):
    ...
    Supports bcrypt (current), PBKDF2, and legacy SHA-256 for migration.
    ...
    # Legacy: plain SHA-256 (unsalted, oldest accounts)
    return stored_hash == hashlib.sha256(password.encode()).hexdigest()

def _needs_rehash(stored_hash):
    """Returns True if the hash should be upgraded to bcrypt."""
    return not stored_hash.startswith(('$2b$', '$2a$'))

def upgrade_password_hash(user_id, password):
    """Re-hashes the password with bcrypt and writes it to the database."""
...
def send_otp_email(recipient_email: str, otp_code: str, username: str) -> tuple[bool, str]:
    """
    Sends the OTP to recipient_email via Gmail SMTP.
          ...
          <tr>
            <td style="background:linear-gradient(135deg,#0D1F2D 0%,#0A2540 100%);padding:32px 40px;text-align:center;border-bottom:1px solid #1E2D3D;">
              <div style="display:inline-block;text-align:center;">
                <span style="font-size:32px;font-weight:900;letter-spacing:-0.5px;font-family:'Segoe UI',Arial,sans-serif;">
                  <span style="color:#FFFFFF;">FINE</span><span style="color:#00D4AA;">LYT</span>
                </span>
              </div>
              <p style="color:#64748B;font-size:12px;margin:8px 0 0 0;letter-spacing:4px;font-family:'Segoe UI',Arial,sans-serif;">
...
"""
    ...
    try:
        ...
        return False, "Email delivery failed. Please contact support."
    except smtplib.SMTPConnectError as exc:
        logging.error("SMTP connection error: %s", exc)
        return False, "Email delivery failed. Please try again later."
    except Exception as exc:
        logging.error("Email delivery error: %s", exc)
        return False, "Email delivery failed. Please try again later."
...
def login_user(identifier, password):
    ...
    if user is None and '@' in identifier:
        cur.execute('SELECT * FROM users WHERE lower(email) = lower(?)', (identifier,))
        user = cur.fetchone()
    conn.close()
    ...
    if not verify_password(password, stored_hash):
        ...
        if attempts >= 5:
            return False, None, "LOCKED_OUT:900"
    ...
    # Auto-upgrade legacy password hashes to bcrypt on successful login
    if _needs_rehash(stored_hash):
        try:
            upgrade_password_hash(user_id, password)
            logging.info("Upgraded password hash to bcrypt for user_id=%s", user_id)
        except Exception as exc:
            logging.warning("Failed to upgrade password hash for user_id=%s: %s", user_id, exc)

    return True, user_id, "Login successful!"

def _register_failed_attempt(identifier):
    """Records a failed login attempt and locks the account after 5 failures for 15 minutes."""
    attempts_info = get_login_attempts(identifier)
...
def check_rate_limit(action: str, max_requests: int, window_seconds: int) -> tuple[bool, str]:
    """
    Session-level rate limiter for Streamlit apps.

    Tracks timestamps of recent actions in st.session_state and returns
    (True, "") if the action is allowed, or (False, error_message) if
    the rate limit has been exceeded.

    Parameters
    ----------
    action         : A unique identifier for the action (e.g. 'login', 'ai_chat').
    ...
    st.session_state[key] = [
        ts for ts in st.session_state[key] if now - ts < window_seconds
    ]

    if len(st.session_state[key]) >= max_requests:
        oldest = min(st.session_state[key])
        retry_after = int(window_seconds - (now - oldest)) + 1
        return False, f"Too many requests. Please wait {retry_after} seconds before trying again."
